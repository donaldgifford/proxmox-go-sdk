...
written-but-unverified here. **Phase 2 (compute) is underway** — tasks 1–2 done:
QEMU list/status/config-get-set/create/clone/delete, plus power
(start/stop/shutdown/reboot/suspend/resume). Power ops POST to `/status/{verb}`
via a shared `statusAction` helper and use the **action-option pattern**: per-op
functional options (`StopOption`, `ShutdownOption`, `SuspendOption`) write to an
unexported `powerConfig` so the `url.Values` wire form never leaks into public
signatures (the template for every later service's action-style ops; distinct
option types keep an op-irrelevant flag from compiling). **Landed so far:**
`types` (`PVEBool`, ID/ref primitives), `pverr` (error taxonomy + `Classify`),
and `api` (transport `DoRequest`/`ExpandPath`, sticky cluster failover, the
three `Credentials` strategies), and `version` (`Capabilities` snapshot,
`AtLeast` + named per-minor gates, `Service` that fetches `/version` and
enforces the 9.0 floor), and `tasks` (UPID parse, `Status`/`Log` reads,
...
land with their services), and `qemu` — the first **service** package and the
template every later service follows (go-architect-designed). The
**service-package pattern**: a
