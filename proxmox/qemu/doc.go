...
// snapshots (Snapshots/CreateSnapshot/RollbackSnapshot/DeleteSnapshot),
// template conversion (ConvertToTemplate — one-way, stopped VMs only; the
// source for linked clones), and the guest agent
// (AgentPing/AgentExec/AgentExecStatus/AgentExecWait). See the package Example
// for a runnable clone → start flow.
//
